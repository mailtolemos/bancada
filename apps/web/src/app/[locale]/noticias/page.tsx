import Link from "next/link";
import { notFound } from "next/navigation";
import { CLUBS, NEWS_SOURCES, getDictionary, isLocale } from "@bancada/core";
import { getNews } from "@/lib/data";
import { NewsCard } from "@/components/NewsCard";
import { SectionHeader } from "@/components/SectionHeader";

export const dynamic = "force-dynamic";

export default async function NewsPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string }>;
  searchParams: Promise<{ fonte?: string; clube?: string }>;
}) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const dict = getDictionary(locale);
  const { fonte, clube } = await searchParams;

  const news = await getNews({ source: fonte, club: clube }).catch(() => []);
  const base = `/${locale}/noticias`;
  const mainClubs = CLUBS.slice(0, 6);

  return (
    <div className="mx-auto max-w-3xl space-y-4">
      <SectionHeader title={`📰 ${dict.news.title}`} />

      {/* Filtros por fonte */}
      <div className="flex flex-wrap gap-1.5">
        <FilterChip href={base} active={!fonte && !clube} label={dict.news.allSources} />
        {NEWS_SOURCES.map((s) => (
          <FilterChip
            key={s.id}
            href={`${base}?fonte=${s.id}`}
            active={fonte === s.id}
            label={s.name}
          />
        ))}
      </div>

      {/* Filtros por clube */}
      <div className="flex flex-wrap gap-1.5">
        {mainClubs.map((c) => (
          <FilterChip
            key={c.slug}
            href={`${base}?clube=${c.slug}`}
            active={clube === c.slug}
            label={c.aliases[0]!.replace(/\b\w/g, (m) => m.toUpperCase())}
          />
        ))}
      </div>

      {news.length ? (
        <div className="grid gap-2.5">
          {news.map((item) => (
            <NewsCard key={item.id} item={item} locale={locale} dict={dict} />
          ))}
        </div>
      ) : (
        <p className="card px-4 py-8 text-center text-sm text-neutral-500">{dict.news.empty}</p>
      )}
    </div>
  );
}

function FilterChip({ href, active, label }: { href: string; active: boolean; label: string }) {
  return (
    <Link
      href={href}
      className={`chip transition-colors ${
        active
          ? "bg-pitch-600 text-white"
          : "bg-neutral-200/80 text-neutral-600 hover:bg-neutral-300 dark:bg-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-700"
      }`}
    >
      {label}
    </Link>
  );
}
